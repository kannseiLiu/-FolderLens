...
1. Exclude directories, symbolic links, empty files, files without stable
   filesystem identity, and repeated paths to the same file identity, then group
   files by byte size. Retain only groups containing at least two files.
...
- Files without stable filesystem identity are left out of trusted duplicate
  estimates instead of risking inflated recoverable-space claims.
